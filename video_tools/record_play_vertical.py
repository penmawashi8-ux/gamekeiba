...
async def main():
    ...
    async with async_playwright() as p:
        ...
        ctx = await browser.new_context(
            ...
            record_video_dir=OUT, record_video_size={"width": 540, "height": 960},
        ...
        await page.wait_for_timeout(2500)
        await page.get_by_text("払い戻し", exact=False).first.scroll_into_view_if_needed()
        ...
        shutil.move(path, f"{OUT}/raw_v.webm")
        await browser.close()

    spy.cancel()
    with open(f"{OUT}/events_v.json", "w") as f:
        json.dump(events, f, ensure_ascii=False)
    print("saved", f"{OUT}/raw_v.webm", len(events), "events")
